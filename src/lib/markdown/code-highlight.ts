...
function processToken(
  ...
  const tokenStyle = getTokenStyles(codeTheme, token.type, options?.forceDarkPalette)
  const content = Array.isArray(token.content)
    ? token.content.map(t => processToken(t, codeTheme, options)).join('')
    : processToken(token.content, codeTheme, options)
...
export function highlightCode(
  ...
  try {
    ...
    return lines.map((line, index) => {
      ...
      const processedLine = lineTokens.map(t => processToken(t, codeTheme, options)).join('')
